...
mongoose.connect(process.env.MONGODB_URI)
  .then(async () => {
    ...
    if (cardCount === 0) {
      const initialCards = [
        { character: "Bimajo", rarity: "◈C", imageUrl: "https://i.imgur.com/0qY1z8u.png" },
        { character: "Jane Elves", rarity: "◈R", imageUrl: "https://i.imgur.com/3kX7z9v.png" },
        { character: "Ru-chan", rarity: "◈E", imageUrl: "https://i.imgur.com/7nZ3x4p.png" },
      ];
...
client.on('interactionCreate', async interaction => {
  ...
  try {
    if (interaction.isCommand()) {
      ...
      console.log(`Button interaction: ${interaction.customId} on messageId ${messageId} by ${interaction.user.tag}`);
      ...
      if (interaction.customId.startsWith('grab_')) {
        ...
        return; // Đảm bảo không tiếp tục xử lý sau khi grab
      }
      ...
      // Kiểm tra nút liên quan đến duel
      ...
      // Kiểm tra các nút khác (như collection)
    ...
    console.error(`Error handling interaction ${interaction.customId || interaction.commandName}:`, error.stack);
